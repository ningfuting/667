...
var initMap = function (time) {
  ...
  map = new ol.Map({
    view: new ol.View({
      zoom: 16,
      minZoom: 2,
      center:  [-8246040.943347618, 4973690.548116669],//[106.51, 29.55],//-8246040.943347618, 4973690.548116669
    }),
    target: 'mapContent',
...
var update = function(time){
  ...
  tileLayer = new ol.layer.Tile({
    source: new ol.source.XYZ({
      url: `http://${config.ip + config.port + config.tiles}?dbname=${timedb}&mode=near&time=${time}&level={z}&col={x}&row={y}`,
...
//数据类型
function changeType(type){  //oceanmap, getile, osmraster
  timedb = type;
  $("#time-reset").click();
  init();
  if(timedb === 'getile'){
    map.getView().setZoom(16);
    map.getView().setCenter([-8244789.287009448, 4534516.824789922])
  }
  if(timedb === 'osmraster'){
    map.getView().setZoom(15);
    map.getView().setCenter([-8246040.943347618, 4973690.548116669])
  }
  if(timedb === 'oceanmap'){
    map.getView().setZoom(5);
    map.getView().setCenter(ol.proj.transform([-74.5,41],'EPSG:4326','EPSG:3857'))
  }
}
window.changeType = changeType
//window.changeType('oceanmap')